...
class SimpleDAOService:
    ...
    def _parse_metta_relief_result(self, metta_result) -> dict:
        """Parse MeTTa DAO relief trigger result - FIXED"""
        try:
            result_data = {
                'triggered': False,
                'relief_amount': 0,
            ...
            if metta_result and len(metta_result) > 0:
                # Handle the raw list format properly
                result_item = metta_result[0]
                
                if isinstance(result_item, dict) and 'raw' in result_item:
                    result_str = result_item['raw']
                elif isinstance(result_item, str):
                    result_str = result_item
                else:
    ...
    def _parse_metta_evaluation_result(self, metta_result) -> dict:
        """Parse MeTTa evaluation result - FIXED"""
        try:
            if not metta_result or len(metta_result) == 0:
                return {'evaluated': False, 'score': 0}
            
            # Handle the raw list format: [{"raw":"[(evaluate-proposal \"id\")]","type":"list"}]
            result = metta_result[0]
            if isinstance(result, dict) and 'raw' in result:
                raw_content = result['raw']
                # Extract actual evaluation from the raw content
                if 'evaluate-proposal' in raw_content:
                    return {'evaluated': True, 'score': 0.5}  # Default score
    ...
    def _parse_warning_data(self, warnings) -> list:
        """Parse warning data - FIXED"""
        try:
            parsed_warnings = []
            if isinstance(warnings, list):
                for warning in warnings:
                    if isinstance(warning, dict):
                        # Extract meaningful warning info
                        warning_text = warning.get('message', warning.get('text', warning.get('description', str(warning))))
                        parsed_warnings.append(warning_text)
                    else:
                        parsed_warnings.append(str(warning))
            elif isinstance(warnings, dict):
                # Single warning object
                warning_text = warnings.get('message', warnings.get('text', warnings.get('description', str(warnings))))
                parsed_warnings.append(warning_text)
            else:
                parsed_warnings.append(str(warnings))
            
            return parsed_warnings
        except Exception as e:
            print(f"Error parsing warnings: {e}")
            return ["Warning parsing failed"]

...
try:
    ...
    print("Warning: IPFS client not available, using fallback storage")
...
class IPFSService:
    def __init__(self, ipfs_api_url: str = "/ip4/127.0.0.1/tcp/5001/http"):
        ...
        if not IPFS_AVAILABLE:
            logger.warning("IPFS client not available, using fallback storage")
...
async def upload_metadata_to_ipfs(metadata: Dict[Any, Any]) -> Optional[str]:
    ...
    """
    if not IPFS_AVAILABLE:
        logger.warning("IPFS not available, skipping metadata upload")
        return None
